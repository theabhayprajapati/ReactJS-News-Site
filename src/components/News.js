...
const News = (props) => {
  const capitallizeFirstAlpa = (string) => {
  ...
  document.title = `${capitallizeFirstAlpa(props.category)} - NewsMonkey`

  const updatenews = async () => {
    props.setProgress(20)
    const url = `https://newsapi.org/v2/top-headlines?country=us&category=${props.category}&apiKey=e3553a68781d448b87d4ebd624b4b888&pagesize=${props.pageSize}&page=${page}`
    // setpagJe(page + 1)
    // *1st using the api's
    setloading(true)
    ...
    setarticles(parsedDat.articles)
    settotalResults(parsedDat.totalResults)
    setloading(false)
  ...
  const fetchMoreData = async () => {
    ...
    const url = `https://newsapi.org/v2/top-headlines?country=us&category=${
      props.category
    }&apiKey=e3553a68781d448b87d4ebd624b4b888&pagesize=${props.pageSize}&page=${
      page + 1
    }`
    setpage(page + 1)
    let data = await fetch(url)
    let parsedDat = await data.json()
    setarticles(articles.concat(parsedDat.articles))
    settotalResults(parsedDat.totalResults)
  }
  ...
  return (
    <div className={'container my-3'}>
      ...
      <h2 className="text-center" style={{}}>
        ...
        <u>
          {''}
          {capitallizeFirstAlpa(props.category)} News
        </u>
      </h2>
      <InfiniteScroll
